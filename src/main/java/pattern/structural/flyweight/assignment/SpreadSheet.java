...
public class SpreadSheet {
  ...
  FontStyleService fontStyleService = new FontStyleService(new FontStyleFactory());
  ...
  public void setFontFamily(int row, int col, FontType fontType) {
    ...
    cells[row][col].setFontStyle(fontStyleService.getFontStyle(fontType));
  }
  ...
  private void generateCells() {
    for (var row = 0; row < MAX_ROWS; row++)
      for (var col = 0; col < MAX_COLS; col++) {
        ...
        cell.setFontStyle(fontStyleService.getFontStyle(FontType.TIMES_NEW_ROMAN));
        cells[row][col] = cell;
      }
  }
